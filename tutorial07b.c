        ...
        selection_e color_selection)
{
    ...
    for (int i = 0; i< height; i++)
    {
        ...
        {
            frame_ptr = pFrameRGB->data[0] + (width * i * 3) + (j * 3);
            if (color_selection != SELECTION_RED)
                *frame_ptr = 0;

            frame_ptr++;

            if (color_selection != SELECTION_GREEN)
                *frame_ptr = 0;

            frame_ptr++;

            if (color_selection != SELECTION_BLUE)
                *frame_ptr = 0;
        }
    }
...
static int transform_to_color_thread(void *args) {
    ...
    for (;;)
    {
        if(packet_queue_get(&is->transformq, &packet, 1) < 0) {
            return -1;
        }

        SDL_LockYUVOverlay(vp->bmp);

        ColorPacket *pkt_data = (ColorPacket*)packet.data;
        remove_color(is, pkt_data->frame, pkt_data->pict,
             pkt_data->selection);
...
/* must also free frame */
int queue_picture(VideoState *is, AVFrame *pFrame, double pts) {

    VideoPicture *vp;
    bool picture_ready = true;
    //int dst_pix_fmt;
